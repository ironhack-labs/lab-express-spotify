...
<details>
  ...
  The error "Invalid access token" typically indicates that the token you are using to authenticate your API request is not valid. This can be cause by a few things, such as the token being incorrect, not having the right permissions, or the token/access keys not being included in the request. 
...
2. If you are loading the token string or access keys using `dotenv` make sure that values are included in the `.env` file and that you are using correct names to access them.

    

   For example, if your are storing your Spotify API access keys in the `.env` file like this:
...
<details>
  ...
  You can ignore certain files or directories when using Git by creating a file called `.gitignore` in the root of your repository. Inside of the  file you should list all the files and folder that you want Git to ignore, one per line.
...
<details>
  ...
  In order to render and show the JSON data in a page, you first have to render the view and pass the data to the template. Example:

```js
app.get('/', (req, res) => {
    // JSON mock data
    const data = { name: "Sarah", languages: ["javascipt", "python", "java"] };

    // Render the `index.hbs` view and pass it the `data` object
